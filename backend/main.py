...
@app.post("/api/snapshots/{snapshot_id}/ai-explain")
async def ai_explain_snapshot(snapshot_id: int, db: Session = Depends(get_db)):
    """
    Call Claude claude-sonnet-4-6 to perform deep root cause analysis on the snapshot
    and return specific, actionable fix suggestions with pipeline step configs.
    ...
    try:
        from services.explanation_engine import generate_ai_explanation

        explanations = await generate_ai_explanation(
            step_name=snap.step_name,
            sample_rows=json.loads(snap.sample_json or "[]"),
            schema=json.loads(snap.schema_json or "{}"),
            null_counts=json.loads(snap.null_counts_json or "{}"),
            stats=json.loads(snap.stats_json or "{}"),
            anomalies=json.loads(snap.anomalies_json or "[]"),
            diff=json.loads(snap.diff_json or "{}"),
            api_key=api_key,
        )
